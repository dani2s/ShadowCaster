...
The list of proteomes could be either:
    2.1. Provided by the user (a collection of FASTA files).
    2.2. Automatically retrieved by ShadowCaster from the NCBI ftp site by using script/get_proteomes.py
...
4. Orthology relationships among the query genome and its phylogenetically related species are obtained by the third-party algorithm ORTHOMCL.
This information is used to calculate the 'probability of orthology' between the query genome and each other genome in the phylogenetic shadow. 
